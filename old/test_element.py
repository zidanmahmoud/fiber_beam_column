...
for nsx in range(NSx):
    ...
    section_list.append(Section(gauss_points[nsx], [1., 1.], fiber_list[nsx][:][:], 1e-11))


Nd_a = Node(1, np.array([0,0,0]))
Nd_b = Node(1, np.array([1,0,0]))
fbe = FiberBeam(1, Nd_a, Nd_b, np.array([0,0,1]), section_list)
Nd_a.update_change_in_node_displacement_incr(np.array([0,0,0,0,0,0]))
Nd_b.update_change_in_node_displacement_incr(np.array([0,0,0.001,0,-0.0015,0]))
