...
class Model(object):
    ...
    def add_node(self, id, x, vec):
        ...
        x : ndarray
            Initial position.
        vec : ndarray
            Arbitary input vector which is not parallel with the undeformed beam.

        Examples
        --------
        Add a node with ID `B`:

        >>> model.add_node(id='B', x=[1,1,0], vec=[0,0,1])
        """
        if id in self._nodes:
            raise RuntimeError('The model already contains a node with id {}'.format(id))

        self._nodes[id] = Node(self, id, x, vec)

    def add_beam_element(self, id, node_a, node_b, youngs_modulus, area):
